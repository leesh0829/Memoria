...
## 주간보고
- [주간보고]에서 주차를 선택하고 양식 A/B/C를 토글합니다.
- 양식 A: 할 일/이슈 순. 양식 B: 고객사별 분류 + 제목줄. 양식 C: 주간 실적/차주 계획 골격.
- 양식 C는 집계 구간이 다릅니다. 기본값은 **전주 금요일 ~ 해당 주 목요일**이며, 설정 > 주간보고에서 시작·종료 요일을 바꿀 수 있습니다.
  머리글 문구(`[ 주간 실적 ]`, `[ 차주 계획 ]`)와 설명 머릿말(`o`)도 같은 곳에서 바꿉니다. 설명과 차주 계획 내용은 비워 두므로 직접 채워 넣습니다.
- 양식 C에는 선택한 고객사의 할 일만 들어갑니다. 기본값은 `SLD 자율형공장` 하나이며, 설정 > 주간보고의 "포함할 고객사" 체크 목록에서 바꿉니다.
