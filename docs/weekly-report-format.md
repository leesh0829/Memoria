...
- **선택한 고객사의 업무만 출력한다.** 기본값은 `SLD 자율형공장` 하나(`report.formatC.clientIds`,
  client id를 쉼표로 이어 저장). 미분류 업무는 필터가 걸린 상태에서 제외된다.
  설정 키가 아직 없고 `SLD 자율형공장` 고객사도 없으면(개명/삭제) 필터를 걸지 않는다.
...
- 포함 고객사는 설정 > 주간보고의 체크 목록에서 바꾼다. 모두 해제하면 업무가 한 건도 출력되지 않는다.
